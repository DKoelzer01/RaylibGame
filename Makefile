...
# List all .cpp files explicitly (add more if you add new source files)
CPP_FILES = \
	src/main.cpp \
	src/world.cpp \
	src/cubemarch.cpp \
...
# Always output to game.exe
# Remove use of $(PROJECT_NAME) and $(EXT) for output
#
game.exe: $(OBJ_FILES)
	$(CC) -o game.exe $^ $(CFLAGS) $(INCLUDE_PATHS) $(LDFLAGS) $(LDLIBS) -D$(PLATFORM)
